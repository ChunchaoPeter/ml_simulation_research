...
if __name__ == "__main__":
    ...
    This script:
    1. Initializes the acoustic model with default parameters
    2. Gets the default initial state for 4 targets
    3. Simulates one time step of state transition
    4. Generates an observation from the new state
    """
    # ============================================================
    # Initialize the acoustic model with default parameters
    # ============================================================
    print("="*70)
    print("Acoustic Multi-Target Tracking Model - Example Usage")
    print("="*70)

    model_params = initialize_acoustic_model()

    # ============================================================
    # Get initial state for 4 targets
    # ============================================================
    x0_initial_target_states = model_params['x0_initial_target_states']

    print(f"\nInitial State (shape: {x0_initial_target_states.shape}):")
    print("  Format: [x1, y1, vx1, vy1, x2, y2, vx2, vy2, ...]")
    print(f"  Values:\n{x0_initial_target_states.numpy()}")

    # ============================================================
    # Simulate one time step using TRUE process noise (Q_real)
    # ============================================================
    print("\n" + "-"*70)
    print("Simulating one time step...")
    print("-"*70)

    x_next = state_transition(
        x0_initial_target_states,
        model_params,
        use_real_noise=True  # Use Q_real for realistic trajectory
    )

    print(f"\nNext state after one step (shape: {x_next.shape}):")
    print(f"  Values:\n{x_next.numpy()}")

    # ============================================================
    # Generate observation from the new state
    # ============================================================
